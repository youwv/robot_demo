...
class Env:
    ...
    def reset(self, reset_action):
        ...
        state = np.concatenate([state, state], 1)
        return state
    ...
    def step(self, action):
        ...
        state1 = get_picture()
        state = np.concatenate([self.temp_state, state1], 1)
        # cv2.imwrite(str(self.count)+'.PNG', state)
        self.temp_state = state1
